...
// Load the rollup chain hash from the autogenerated file produced by the rollup build.
#[allow(dead_code)]
mod rollup_autogen {
    include!(concat!(
    ...
    match parsed_call {
        ParsedMidnightCall::Deposit { amount, rho, recipient } => {
            ...
            let pre_auth_data = match extract_pre_authenticated_data(&tx) {
                Ok(data) => {
                    debug!("✓ Extracted pre-authenticated data for deposit (signature already verified)");
                    Some(data)
                }
                Err(e) => {
