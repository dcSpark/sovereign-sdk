...
use sov_ligero_adapter::{Ligero, LigeroCodeCommitment, LigeroVerifier, LigeroVerifiedInputs};
use sov_modules_api::{
...
use midnight_privacy::{CallMessage as MidnightCallMessage, EncryptedNote, FullViewingKey, Hash32 as MidnightHash32, SpendPublic, spend_public_from_verified_args};
...
/// Directory (relative to workspace root) where full midnight
/// transactions are stored as JSON files, one per tx hash.
/// For the rollup-ligero demo this resolves to:
/// `examples/rollup-ligero/demo_data/midnight_tx_json`.
pub const MIDNIGHT_TX_JSON_DIR: &str = "examples/rollup-ligero/demo_data/midnight_tx_json";

/// Compute the JSON file path for a given midnight transaction hash.
pub fn midnight_tx_json_path(tx_hash: &str) -> PathBuf {
    let filename = format!("{tx_hash}.json");
    PathBuf::from(MIDNIGHT_TX_JSON_DIR).join(filename)
}

// Chain hash from demo-rollup/autogenerated.rs (generated at build time).
// Include the autogenerated file directly so we stay in sync with the rollup.
// Note: The node uses demo-stf which includes demo-rollup/autogenerated.rs
mod rollup_autogen {
    include!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../../examples/demo-rollup/autogenerated.rs"
    ));
}
...
async fn verify_and_record_midnight_handler(
    ...
    match parsed_call {
        ...
        ParsedMidnightCall::Transfer {
            ...
            info!(
                "Processing midnight transfer: nullifiers={}, anchor_root=0x{}, proof_size={} bytes",
                nullifiers.len(),
            ...
            info!(
                "✓ Transfer proof verified in {:.2}ms: outputs={}, recipient_atts={:?}, view_atts={:?}",
                metrics.proof_verify_ms,
                proof_public.output_commitments.len(),
                proof_public.recipient_attestations.as_ref().map(|v| v.len()),
                proof_public.view_attestations.as_ref().map(|v| v.len()),
            );
            ...
            metrics.tx_creation_ms = persist_start.elapsed().as_secs_f64() * 1000.0;
            debug!(
                "✓ Stored verified midnight transfer: nullifiers={:?}, anchor_root=0x{}, hash={}, encrypted_notes={}",
                nullifiers.iter().map(hex::encode).collect::<Vec<_>>(),
        ...
        ParsedMidnightCall::Withdraw {
            ...
            debug!(
                "Parsed midnight withdrawal: nullifiers={:?}, anchor_root=0x{}, withdraw_amount={}, proof_size={} bytes, view_ciphertexts={:?}",
                nullifiers.iter().map(hex::encode).collect::<Vec<_>>(),
                hex::encode(anchor_root),
                withdraw_amount,
                proof.len(),
            ...
            metrics.tx_creation_ms = persist_start.elapsed().as_secs_f64() * 1000.0;
            debug!(
                "✓ Stored verified midnight withdrawal: nullifiers={:?}, anchor_root=0x{}, withdraw_amount={}, to={:?}, hash={}, encrypted_notes={}",
                nullifiers.iter().map(hex::encode).collect::<Vec<_>>(),
                hex::encode(anchor_root),
                withdraw_amount,
                to,
...
async fn verify_ligero_proof(
    ...
    let result = tokio::task::spawn_blocking(move || {
        ...
        let verified: LigeroVerifiedInputs = LigeroVerifier::verify_and_get_verified_args(&proof, &method_id)
            .map_err(|e| ServiceError::ProofError(format!("Verification failed: {}", e)))?;

        debug!(
            "Ligero value-setter proof verified with {} args",
            verified.args.len()
        );

        // Extract the proven value from verified args
        // value_validator.wasm ABI: arg[0] = proven value, arg[1] = claimed value (both i32)
        if verified.args.is_empty() {
            return Err(ServiceError::ProofError("No args in verified proof".to_string()));
        }
        
        let proven_value = match &verified.args[0] {
            sov_ligero_adapter::LigeroArg::I64 { i64: v } => *v as u32,
            sov_ligero_adapter::LigeroArg::String { str: s } => s.parse::<u32>()
                .map_err(|e| ServiceError::ProofError(format!("Bad value arg: {}", e)))?,
            sov_ligero_adapter::LigeroArg::Hex { hex: h } => {
                let bytes = hex::decode(h.trim_start_matches("0x"))
                    .map_err(|e| ServiceError::ProofError(format!("Bad hex value: {}", e)))?;
                if bytes.len() != 4 {
                    return Err(ServiceError::ProofError(format!("Expected 4-byte value, got {}", bytes.len())));
                }
                u32::from_le_bytes([bytes[0], bytes[1], bytes[2], bytes[3]])
            }
        };

        // Check that the verified value matches the claimed value
...
> {
    match parse_midnight_call(tx)? {
        ParsedMidnightCall::Withdraw {
            ...
            Ok((proof, anchor_root, nullifiers, withdraw_amount, to, view_ciphertexts))
        }
...
pub fn verify_midnight_transaction_signature(
    ...
    info!("Verifying signature with chain_hash: 0x{}", hex::encode(ROLLUP_CHAIN_HASH));
    info!("Transaction hash: {}", tx.hash());
    let mut meter = UnlimitedGasMeter::<RollupSpec>::default();
    tx.verify(&ROLLUP_CHAIN_HASH, &mut meter)
        .map_err(|e| ServiceError::SignatureError(format!("Invalid signature: {}", e)))
}
...
pub async fn verify_midnight_withdraw_proof(
    ...
    expected_nullifiers: Vec<MidnightHash32>,
    ...
    tokio::task::spawn_blocking(move || {
        ...
        info!("Configuring Ligero environment for midnight proof verification...");
        configure_ligero_env_for_midnight()?;
        
        info!("Starting Ligero proof verification (proof size: {} bytes)", proof_vec.len());
        
        // SECURITY: Use verify_and_get_verified_args() instead of verify().
        // The verify() method returns T by deserializing public_output, which is NOT
        // cryptographically bound to the proof. An attacker could modify public_output
        // while keeping the proof valid. verify_and_get_verified_args() returns only
        // the args that were actually verified by Ligero.
        let verified: LigeroVerifiedInputs = LigeroVerifier::verify_and_get_verified_args(&proof_vec, &method_id)
            .map_err(|e| {
                error!("Ligero verification failed: {}", e);
                ServiceError::ProofError(format!("Verification failed: {}", e))
            })?;

        info!(
            "✓ Midnight proof verified successfully: {} args, {} private indices",
            verified.args.len(),
            verified.private_indices.len()
        );
        
        // Log first few args for debugging
        for (idx, arg) in verified.args.iter().enumerate().take(10) {
            debug!("  arg[{}]: {:?}", idx, arg);
        }
        if verified.args.len() > 10 {
            debug!("  ... and {} more args", verified.args.len() - 10);
        }

        // Parse SpendPublic from the verified args (not from unverified public_output)
        info!("Parsing SpendPublic from {} verified args...", verified.args.len());
        let public: SpendPublic = spend_public_from_verified_args(&verified.args)
            .map_err(|e| {
                error!("Failed to parse SpendPublic from verified args: {}", e);
                ServiceError::ProofError(format!("Failed to parse verified args: {}", e))
            })?;
        
        info!(
            "✓ Parsed SpendPublic: anchor=0x{}, nullifiers={}, outputs={}, withdraw={}",
            hex::encode(&public.anchor_root[..8]),
        ...
        if public.nullifiers != expected_nullifiers {
            return Err(ServiceError::ProofError(format!(
                "Nullifiers mismatch: expected {:?}, proof {:?}",
                expected_nullifiers.iter().map(hex::encode).collect::<Vec<_>>(),
                ...
                MidnightCallMessage::Transfer {
                    ...
                    serde_json::json!({
                        "transfer": {
                            ...
                            "nullifiers": nullifiers.iter().map(hex::encode).collect::<Vec<_>>(),
            ...
            let lightweight_runtime_call = {
                ...
                match &v0.runtime_call {
                    RuntimeCall::MidnightPrivacy(midnight_call) => {
                        match midnight_call {
                            CallMessage::Withdraw { proof: _, anchor_root, nullifiers, withdraw_amount, to, view_ciphertexts, recipient_ciphertexts, gas } => {
                                ...
                                RuntimeCall::MidnightPrivacy(CallMessage::Withdraw {
                                    ...
                                    recipient_ciphertexts: recipient_ciphertexts.clone(),
                                    gas: gas.clone(),
                                })
                            }
                            CallMessage::Transfer { proof: _, anchor_root, nullifiers, view_ciphertexts, recipient_ciphertexts, gas } => {
...
pub async fn store_verified_midnight_transaction(
    ...
    // Persist the full transaction blob to a JSON file on disk instead of the database.
    // This avoids storing large (~3MB) blobs in the shared DA database while keeping
    // a deterministic reference (tx_hash.json) for debugging and replay.
    let json_path = midnight_tx_json_path(tx_hash);
    if let Some(parent) = json_path.parent() {
        tokio::fs::create_dir_all(parent).await.map_err(|err| {
            ServiceError::Internal(format!(
                "Failed to create midnight_tx_json directory {}: {err}",
                parent.display()
            ))
        })?;
    }
    ...
    let file_payload = serde_json::json!({
        "tx_hash": tx_hash,
        "full_transaction_blob": full_transaction_blob,
    });

    let file_bytes = serde_json::to_vec_pretty(&file_payload).map_err(|err| {
        ServiceError::Internal(format!(
            "Failed to serialize midnight transaction JSON for {}: {err}",
            tx_hash
        ))
    })?;

    tokio::fs::write(&json_path, file_bytes).await.map_err(|err| {
        ServiceError::Internal(format!(
            "Failed to write midnight transaction JSON file {}: {err}",
            json_path.display()
        ))
    })?;
