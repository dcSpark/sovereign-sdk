            ...
            tokio::task::spawn_blocking(
                ...
                let (view_attestations, viewer_data_list) = if let Some(fvk) = viewer_fvk {
                    ...
                    let (pay_att, _pay_enc) = make_viewer_bundle(
                        &fvk, &DOMAIN, pay_value, &pay_rho, &pay_recipient, &sender_id_out, &cm_ins, &cm_pay,
                    )?;
                    if has_change {
                        let (change_att, _change_enc) = make_viewer_bundle(
                            &fvk, &DOMAIN, change_value, &change_rho, &change_recipient, &sender_id_out, &cm_ins, &cm_change,
                        )?;
            ...
            move || -> anyhow::Result<BuiltTransfer> {
                ...
                let view_ciphertexts: Option<Vec<EncryptedNote>> = match viewer_fvk {
                    Some(fvk) => {
                        ...
                        let in_value_u64: u64 = wallet.value.try_into().context(
                            "input value does not fit into u64",
                        )?;
