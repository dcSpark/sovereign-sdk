...
/// Test emulates access patterns to [`StorableMidnightDaService`] in a regular rollup.
...
fn bench_storable_midnight_da_service(c: &mut Criterion) {
    ...
    println!("Setting up MidnightDA and wait for {original_blocks} blocks to be produced");
    let (da_service, handles) = rt.block_on(async {
        let da_service = StorableMidnightDaService::from_config(
            MidnightDaConfig {
                connection_string: format!("sqlite://{}?mode=rwc", path.display()),
                sender_address: Default::default(),
                finalization_blocks: 0,
                block_producing: sov_midnight_da::BlockProducingConfig::Periodic {
    ...
    let mut group = c.benchmark_group("StorableMidnightDaService");
...
criterion_group!(benches, bench_storable_midnight_da_service);
