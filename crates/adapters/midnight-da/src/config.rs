...
pub(crate) const GENESIS_HEADER: MidnightBlockHeader = MidnightBlockHeader {
    prev_hash: MidnightHash([0; 32]),
    hash: MidnightHash([1; 32]),
    height: 0,
    // 2023-01-01T00:00:00Z
    time: Time::from_millis(1672531200000),
};

pub(crate) const GENESIS_BLOCK: MidnightBlock = MidnightBlock {
...
/// The configuration for Midnight Da.
#[derive(Debug, Clone, serde::Deserialize, serde::Serialize, JsonSchema)]
pub struct MidnightDaConfig {
...
impl MidnightDaConfig {
    /// Create [`MidnightDaConfig`] with instant finality.
    pub fn instant_with_sender(sender: MidnightAddress) -> Self {
        MidnightDaConfig {
    ...
    pub fn sqlite_in_dir(dir: impl AsRef<std::path::Path>) -> anyhow::Result<String> {
        ...
        let db_path = path.join("midnight_da.sqlite");
        tracing::debug!(path = %db_path.display(), "Opening StorableMidnightDa");
    ...
    /// Instance of [`MidnightDaConfig`] that resembles Solana DA. Batch production is periodic.
    pub fn solana_like(connection_string: String, sender: MidnightAddress, seed: HexHash) -> Self {
        MidnightDaConfig {
            connection_string,
            sender_address: sender,
...
mod tests {
    ...
    fn periodic_block_producing() {
        let config_s = r#"
            ...
            block_time_ms = 1_000
        "#;
        let config = toml::from_str::<MidnightDaConfig>(config_s).unwrap();
