...
pub use address::{MidnightAddress, MOCK_SEQUENCER_DA_ADDRESS};
...
pub struct MidnightHash(pub [u8; 32]);

impl Debug for MidnightHash {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}", HexHash::new(self.0))
    }
}

impl core::fmt::Display for MidnightHash {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
        write!(f, "{}", HexHash::new(self.0))
    }
}

impl core::str::FromStr for MidnightHash {
    type Err = anyhow::Error;

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let inner = HexHash::from_str(s)?;
        Ok(MidnightHash(inner.0))
    }
}

impl AsRef<[u8]> for MidnightHash {
    fn as_ref(&self) -> &[u8] {
        &self.0
    }
}

impl TryFrom<Vec<u8>> for MidnightHash {
    ...
    fn try_from(value: Vec<u8>) -> Result<Self, Self::Error> {
        ...
        Ok(MidnightHash(hash))
    }
}

impl BlockHashTrait for MidnightHash {}

/// A mock block header used for testing.
#[derive(Serialize, Deserialize, PartialEq, Debug, Clone, derive_more::Display)]
#[display("{:?}", self)]
pub struct MidnightBlockHeader {
    /// The height of this block.
    pub height: u64,
    /// The hash of the previous block.
    pub prev_hash: MidnightHash,
    /// The hash of this block.
    pub hash: MidnightHash,
    /// The time at which this block was created.
    pub time: Time,
}

impl MidnightBlockHeader {
    /// Generates [`MidnightBlockHeader`] with given height & time, where hashes are derived from height.
    /// Can be used in tests, where a header of the following blocks will be consistent.
    pub fn new(height: u64, time: Time) -> MidnightBlockHeader {
        let prev_hash = u64_to_bytes(height);
        let hash = u64_to_bytes(height + 1);
        MidnightBlockHeader {
            height,
            hash: MidnightHash(hash),
            prev_hash: MidnightHash(prev_hash),
            time,
        }
    }

    /// Generates [`MidnightBlockHeader`] with given height, where hashes are derived from height.
    /// Can be used in tests, where a header of the following blocks will be consistent.
    pub fn from_height(height: u64) -> MidnightBlockHeader {
        Self::new(height, Time::now())
    }
}

impl Default for MidnightBlockHeader {
    fn default() -> Self {
        MidnightBlockHeader::from_height(0)
    }
}

impl BlockHeaderTrait for MidnightBlockHeader {
    type Hash = MidnightHash;
...
pub struct MidnightBlob {
    pub(crate) address: MidnightAddress,
...
impl MidnightBlob {
    ...
    pub fn new(tx_blob: Vec<u8>, address: MidnightAddress, hash: [u8; 32]) -> Self {
        Self {
            address,
    ...
    pub fn new_with_hash(blob: Vec<u8>, address: MidnightAddress) -> Self {
        let data_hash = hash_to_array(&blob).to_vec();
        let blob_hash = MidnightHash(hash_to_array(&data_hash));
        Self {
            address,
...
pub struct MidnightBlock {
    ...
    pub header: MidnightBlockHeader,
    /// Rollup's batch namespace.
    pub batch_blobs: Vec<MidnightBlob>,
    /// Rollup's proof namespace.
    pub proof_blobs: Vec<MidnightBlob>,
}

#[cfg(feature = "native")]
impl sov_rollup_interface::node::da::SlotData for MidnightBlock {
...
impl MidnightBlock {
    ...
    pub fn next_mock(&self) -> MidnightBlock {
    ...
    pub fn default_at_height(height: u64) -> Self {
        MidnightBlock {
            header: MidnightBlockHeader::from_height(height),
            ..Default::default()
        }
    }
    ...
    pub fn as_relevant_blobs(&self) -> RelevantBlobs<MidnightBlob> {
        RelevantBlobs {
...
mod tests {
    ...
    fn header_to_string() {
        let header = MidnightBlockHeader {
            prev_hash: MidnightHash([1; 32]),
            hash: MidnightHash([2; 32]),
            height: 1,
            time: Time::from_secs(1672531200),
        };

        let expected = "MidnightBlockHeader { height: 1, prev_hash: 0x0101010101010101010101010101010101010101010101010101010101010101, hash: 0x0202020202020202020202020202020202020202020202020202020202020202, time: Time { millis: 1672531200000 } }";
