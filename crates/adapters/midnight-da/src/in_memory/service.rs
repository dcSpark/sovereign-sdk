...
#[derive(Clone, Debug)]
pub struct MidnightDaService {
    sequencer_da_address: MidnightAddress,
    aggregated_proof_buffer: Arc<Mutex<VecDeque<MidnightBlob>>>,
    blocks: Arc<RwLock<VecDeque<MidnightBlock>>>,
    ...
    finalized_header_sender: broadcast::Sender<MidnightBlockHeader>,
...
type HeaderStream = BoxStream<'static, Result<MidnightBlockHeader, anyhow::Error>>;

impl MidnightDaService {
    /// Creates a new [`MidnightDaService`] with instant finality.
    pub fn new(sequencer_da_address: MidnightAddress) -> Self {
        ...
        let mut blocks: VecDeque<MidnightBlock> = Default::default();
    ...
    pub fn sequencer_address(&self) -> MidnightAddress {
    ...
    async fn get_last_finalized_height(&self, blocks: &VecDeque<MidnightBlock>) -> u64 {
        blocks
            .len()
    ...
    fn make_blob(&self, blob: Vec<u8>) -> MidnightBlob {
        MidnightBlob::new_with_hash(blob, self.sequencer_da_address)
    }

    fn make_new_block(
        &self,
        batch_blob: MidnightBlob,
        proof_blobs: Vec<MidnightBlob>,
        blocks: &mut VecDeque<MidnightBlock>,
    ) -> MidnightBlock {
        let prev = blocks
            .iter()
            .last()
    ...
    /// In the [`MidnightDaService`] a single block contains only one batch blob and any number of proof blobs.
    fn add_block(
        &self,
        batch_blob: MidnightBlob,
        proof_blob: Vec<MidnightBlob>,
        blocks: &mut VecDeque<MidnightBlock>,
    ) -> (u64, MidnightHash) {
...
fn block_hash(height: u64, blob_hashes: &[MidnightHash], prev_hash: [u8; 32]) -> MidnightHash {
    ...
    MidnightHash::from(hash_to_array(&block_to_hash))
}

#[async_trait]
impl DaService for MidnightDaService {
    type Spec = MidnightDaSpec;
    type Config = MidnightDaConfig;
    type Verifier = MidnightDaVerifier;
    type FilteredBlock = MidnightBlock;
    ...
    /// Sends proof to the MidnightDA. The submitted proof is internally buffered and will be included on the MidnightDA
...
mod tests {
    ...
    async fn test_empty() {
        let mut da = MidnightDaService::new(MidnightAddress::new([1; 32]));
        da.wait_attempts = 10;
    ...
    async fn get_finalized_headers_collector(
        ...
        // But we need to wait longer, as `MidnightDa
    ...
    fn validate_get_finalized_header_response(
        ...
        response: anyhow::Result<MidnightBlockHeader>,
    ) {
    ...
    mod non_instant_finality {
        ...
        async fn read_multiple_times() -> anyhow::Result<()> {
            let mut da = MidnightDaService::new(MidnightAddress::new([1; 32])).with_finality(4);
            da.wait_attempts = 2;
    ...
    async fn test_zk_submission() -> anyhow::Result<()> {
        let da = MidnightDaService::new(MidnightAddress::new([1; 32]));
    ...
    mod reo4g_control {
        ...
        async fn test_reorg_control_success() -> anyhow::Result<()> {
            let da = MidnightDaService::new(MidnightAddress::new([1; 32])).with_finality(4);
        ...
        async fn test_attempt_reorg_after_finalized() -> anyhow::Result<()> {
            let da = MidnightDaService::new(MidnightAddress::new([1; 32])).with_finality(3);
    ...
    fn assert_consecutive_blocks(block1: &MidnightBlock, block2: &MidnightBlock) {
