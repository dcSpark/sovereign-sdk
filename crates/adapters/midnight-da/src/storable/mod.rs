...
mod tests {
    ...
    async fn manually_triggered_blocks_are_fetched_after_await() -> anyhow::Result<()> {
        ...
        let config = MidnightDaConfig {
            connection_string: MidnightDaConfig::sqlite_in_memory(),
        ...
        let da_service = StorableMidnightDaService::from_config(config, shutdown_receiver).await;
    ...
    async fn manually_produce_blocks_from_different_sender_with_timestamp() -> anyhow::Result<()> {
        ...
        let da_layer = Arc::new(RwLock::new(
            StorableMidnightDaLayer::new_in_path(tempdir.path(), 0).await?,
        ));

        let sender_1 = MidnightAddress::new([0; 32]);
        let da_service_1 =
            StorableMidnightDaService::new_manual_producing(sender_1, da_layer.clone()).await;
        let sender_2 = MidnightAddress::new([1; 32]);
        let da_service_2 =
            StorableMidnightDaService::new_manual_producing(sender_2, da_layer.clone()).await;
    ...
    impl TestDaSender {
        fn address(&self) -> MidnightAddress {
            match self {
                TestDaSender::One => MidnightAddress::new([0; 32]),
                TestDaSender::Two => MidnightAddress::new([1; 32]),
                TestDaSender::Three => MidnightAddress::new([2; 32]),
            }
        }

        async fn build_da_services(
            da_layer: Arc<RwLock<StorableMidnightDaLayer>>,
        ) -> HashMap<TestDaSender, StorableMidnightDaService> {
            let mut da_services: HashMap<TestDaSender, StorableMidnightDaService> = HashMap::new();
