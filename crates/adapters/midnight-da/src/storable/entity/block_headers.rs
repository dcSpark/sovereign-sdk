...
use crate::{MidnightBlockHeader, MidnightHash};
...
impl From<Model> for MidnightBlockHeader {
    fn from(value: Model) -> Self {
        let hash = MidnightHash::try_from(value.hash).expect("Corrupted `hash` in database");
        let prev_hash =
            MidnightHash::try_from(value.prev_hash).expect("Corrupted `prev_hash` in database");
...
impl From<MidnightBlockHeader> for ActiveModel {
    fn from(block_header: MidnightBlockHeader) -> Self {
