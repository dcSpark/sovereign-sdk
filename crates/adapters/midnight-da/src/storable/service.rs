//! Data Availability service is a controller of [`StorableMidnightDaLayer`].
...
/// DaService that works on top of [`StorableMidnightDaLayer`].
#[derive(Clone)]
pub struct StorableMidnightDaService {
    /// The address of the sequencer.
    pub sequencer_da_address: MidnightAddress,
    da_layer: Arc<RwLock<StorableMidnightDaLayer>>,
    block_producing: BlockProducingConfig,
    aggregated_proof_sender: broadcast::Sender<()>,
    head_block: watch::Receiver<MidnightBlockHeader>,
...
impl StorableMidnightDaService {
    async fn construct(
    ...
    /// Returns a handle to the `StorableMidnightDaLayer` backing this service.
    pub fn da_layer(&self) -> Arc<RwLock<StorableMidnightDaLayer>> {
        self.da_layer.clone()
    }
    ...
    /// Create a new [` StorableMidnightDaService `] with the given address.
    pub async fn new(
        sequencer_da_address: MidnightAddress,
        da_layer: Arc<RwLock<StorableMidnightDaLayer>>,
        block_producing: BlockProducingConfig,
    ...
    /// Create a new [` StorableMidnightDaService `] with the given address and [`BlockProducingConfig::Manual`].
    /// Shorter constructor.
    ...
    /// Creates a new instance with different address, but on the same [`StorableMidnightDaLayer`].
    /// Block production of this new instance is manual.
    /// Panics if passed address is the same as the original one.
    pub async fn another_on_the_same_layer(&self, new_da_address: MidnightAddress) -> Self {
    ...
    /// Creates new [`StorableMidnightDaService`] with a given address.
            ...
            None => {
                ...
                if let Some(randomization) = &config.randomization {
                    tracing::debug!(
                        ...
                        "StorableMidnightDaLayer will have randomizer"
                    );
    ...
    /// Trigger creation of a new block on underlying [`StorableMidnightDaLayer`].
    pub async fn produce_block_now(&self) -> anyhow::Result<()> {
        let mut da_layer = self.da_layer.write().await;
        da_layer.produce_block().await
    }

    /// Wrapper around [`StorableMidnightDaService::produce_block_now`] to quickly
    ...
    /// Sets randomized blob retrieval by adjust [`Randomizer`] in underlying [`StorableMidnightDaLayer`].
    ...
    /// Re-org simulation: Rewinds the underlying [`StorableMidnightDaLayer`] to the specified height.
    /// Refer to [`StorableMidnightDaLayer::rewind_to_height`] for more details.
...
#[async_trait]
impl DaService for StorableMidnightDaService {
    ...
    async fn get_block_at(&self, height: u64) -> Result<Self::FilteredBlock, Self::Error> {
        ...
        if height > u32::MAX as u64 {
            return Err(anyhow::anyhow!(
                "Height {} is too big for StorableMidnightDaService. Max is {}",
                height,
                u32::MAX
            ));
        }
    ...
    > {
        ...
        if !self.send_transaction_success.load(Ordering::Relaxed) {
            tx.send(Err(anyhow::anyhow!(
                "StorableMidnightDaService::send_transaction failed"
            )))
            .unwrap();
            return rx;
        }
...
mod tests {
    ...
    async fn check_consistency(
        da_service: &StorableMidnightDaService,
    ...
    async fn querying_height_above_u32_max() -> anyhow::Result<()> {
        ...
        assert_eq!(
            "Height 4294967296 is too big for StorableMidnightDaService. Max is 4294967295",
            err
        );
