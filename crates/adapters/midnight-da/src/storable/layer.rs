...
#[derive(Debug)]
pub struct StorableMidnightDaLayer {
    ...
    pub(crate) finalized_header_sender: broadcast::Sender<MidnightBlockHeader>,
    head_header_sender: watch::Sender<MidnightBlockHeader>,
    randomizer: Option<Randomizer>,
}

impl StorableMidnightDaLayer {
    /// Creates new [`StorableMidnightDaLayer`] by passing connections string directly to [`Database`]
    ...
    pub async fn new_in_path(
        ...
        let connection_string = MidnightDaConfig::sqlite_in_dir(path)?;
    ...
    pub async fn produce_block_with_timestamp(
        ...
        let new_head = MidnightBlockHeader {
            height: self.next_height as u64,
            prev_hash: MidnightHash(prev_block_hash),
            hash: MidnightHash(this_block_hash),
            time: timestamp,
        };
    ...
    async fn get_header_at(&self, height: u32) -> anyhow::Result<MidnightBlockHeader> {
        if height < 1 {
    ...
    pub async fn get_head_block_header(&self) -> anyhow::Result<MidnightBlockHeader> {
        self.get_header_at(self.next_height.saturating_sub(1)).await
    }

    /// Get updates on the latest head block
    pub fn subscribe_to_head_updates(&self) -> watch::Receiver<MidnightBlockHeader> {
    ...
    pub(crate) async fn get_block_at(&self, height: u32) -> anyhow::Result<MidnightBlock> {
        if height >= self.next_height {
            anyhow::bail!("Block at height {} has not been produced yet", height);
        }
        if height == 0 {
        ...
        for blob in blobs {
            match blob.namespace.as_str() {
                entity::BATCH_NAMESPACE => batch_blobs.push(MidnightBlob::from(blob)),
                entity::PROOF_NAMESPACE => proof_blobs.push(MidnightBlob::from(blob)),
                namespace => {
    ...
    pub async fn rewind_to_height(&mut self, height: u32) -> anyhow::Result<()> {
        ...
        tracing::info!(
            past_next_height,
            next_height = self.next_height,
            "StorableMidnightDaLayer rewound"
        );
...
/// Controller of the randomization behaviour for [`StorableMidnightDaLayer`].
...
impl Randomizer {
    ...
    // Note: it should not call `StorableMidnightDaLayer::produce_block` because it will lead to infinite recursion.
    async fn produce_block(
        &mut self,
        da_layer: &mut StorableMidnightDaLayer,
        timestamp: sov_rollup_interface::da::Time,
    ) -> anyhow::Result<()> {
...
mod tests {
    ...
    const DEFAULT_SENDER: MidnightAddress = MidnightAddress::new([1; 32]);
    ...
    async fn check_da_layer_consistency(da_layer: &StorableMidnightDaLayer) -> anyhow::Result<()> {
    ...
    async fn check_expected_blobs(
        da_layer: &StorableMidnightDaLayer,
        expected_blocks: &[Vec<(TestBlob, MidnightAddress)>],
    ) -> anyhow::Result<()> {
    ...
    fn get_finalized_headers_collector(
        da: &StorableMidnightDaLayer,
        expected_num_headers: usize,
    ) -> JoinHandle<Vec<MidnightBlockHeader>> {
    ...
    async fn submit_blobs_and_restart(
        connection_string: &str,
        blocks: Vec<Vec<(TestBlob, MidnightAddress)>>,
    ) -> anyhow::Result<()> {
    ...
    fn check_block_batch(block: &mut MidnightBlock, idx: usize, expected: &[u8]) {
        let batch = block.batch_blobs.get_mut(idx).unwrap();
        assert_eq!(expected, batch.full_data());
    }

    fn check_block_proof(block: &mut MidnightBlock, idx: usize, expected: &[u8]) {
    ...
    async fn empty_layer() -> anyhow::Result<()> {
        ...
        let da_layer = StorableMidnightDaLayer::new_in_path(tempdir.path(), 0).await?;
    ...
    async fn submit_batches_and_restart_regular_sqlite() -> anyhow::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let db_path = tempdir.path().join("midnight_da.sqlite");
        let connection_string = format!("sqlite://{}?mode=rwc", db_path.to_string_lossy());

        let sender_1 = MidnightAddress::new([1; 32]);
        let sender_2 = MidnightAddress::new([2; 32]);
    ...
    async fn submit_batches_and_proofs_and_restart_regular() -> anyhow::Result<()> {
        ...
        let sender_1 = MidnightAddress::new([1; 32]);
        let sender_2 = MidnightAddress::new([2; 32]);
        let sender_3 = MidnightAddress::new([3; 32]);
    ...
    async fn close_before_producing_block() -> anyhow::Result<()> {
        ...
        {
            let mut da_layer = StorableMidnightDaLayer::new_in_path(tempdir.path(), 0).await?;
            check_da_layer_consistency(&da_layer).await?;
    ...
    #[tokio::test(flavor = "multi_thread")]
    #[ignore]
    async fn generate_midnight_da_with_many_empty_blocks() -> anyhow::Result<()> {
        ...
        let mut layer = StorableMidnightDaLayer::new_from_connection(&connection_string, 0).await?;
        for _ in 0..10_000 {
    ...
    /// checking that [`StorableMidnightDaLayer`] can return blobs out of order if a related option is set.
    #[tokio::test(flavor = "multi_thread")]
    async fn blobs_out_of_order_works_for_single_block() -> anyhow::Result<()> {
        ...
        let mut da_layer = StorableMidnightDaLayer::new_in_memory(1).await?;
    ...
    fn get_raw_data(mut block: MidnightBlock) -> (Vec<Vec<u8>>, Vec<Vec<u8>>) {
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn storable_midnight_da_rewinds_and_replaces_blobs() -> anyhow::Result<()> {
        let finality = 10;
        let end_height = 15;
        // The height **after** which new fork is going to be built.
        let fork_height = 5;
        let mut da_layer = StorableMidnightDaLayer::new_in_memory(finality).await?;
    ...
    ///
    /// By design, StorableMidnightDaLayer only send notification about the last finalized height.
    ///
    #[tokio::test(flavor = "multi_thread")]
    async fn finalization_notifications_are_skipped_when_parameter_changes() -> anyhow::Result<()> {
        ...
        {
            let mut da_layer =
                StorableMidnightDaLayer::new_in_path(tempdir.path(), initial_finality).await?;
        ...
        {
            let mut da_layer =
                StorableMidnightDaLayer::new_in_path(tempdir.path(), changed_finality).await?;
            let mut rx = da_layer.finalized_header_sender.subscribe();
    ...
    async fn reshuffling_test(
        ...
        let mut da_layer = StorableMidnightDaLayer::new_in_memory(finality_blocks).await?;
        let head_block_receiver = da_layer.subscribe_to_head_updates();
    ...
    async fn test_reorg_interval_works() -> anyhow::Result<()> {
        ...
        let blob = [10; 10];
        let mut seen_blocks: Vec<MidnightBlockHeader> = Vec::new();
    ...
    async fn test_rewind_and_extend() -> anyhow::Result<()> {
        // std::env::set_var("RUST_LOG", "debug,sov_midnight_da=trace");
