...
#[derive(Debug, Error)]
pub enum SetValueMidnightPrivacyError<S: Spec> {
...
impl<S: Spec> ValueMidnightPrivacy<S> {
    ...
    pub(crate) fn set_value_with_proof(
        ...
        #[cfg(feature = "native")]
        {
            ...
            let public: ValueProofPublic = LigeroVerifier::verify(&proof, &method_id)
                .map_err(|e| SetValueMidnightPrivacyError::<S>::ProofVerificationFailed(e.to_string()))?;
            
            // Ensure the verified journal matches the requested value
            if public.value != value {
                return Err(SetValueMidnightPrivacyError::<S>::ValueMismatch {
    ...
    pub(crate) fn update_method_id(
        ...
        if &admin != context.sender() {
            return Err(SetValueMidnightPrivacyError::WrongSender::<S> {
                admin,
