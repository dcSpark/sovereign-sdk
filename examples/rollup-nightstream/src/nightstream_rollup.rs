...
///
/// Uses `StorableMidnightDaService` which automatically calls
/// `set_shared_db_connection_string()` during `from_config()`, making the DA
/// database available to the sequencer worker-tx endpoint and the
/// `midnight-privacy` pre-verified hydrator without env-var workarounds.
#[derive(Default)]
pub struct NightstreamRollup<M> {
    phantom: std::marker::PhantomData<M>,
}

/// The rollup spec: Midnight DA with MockZkvm for both inner and outer VM.
pub type NightstreamRollupSpec<M> =
    ConfigurableSpec<MidnightDaSpec, MockZkvm, MockZkvm, MultiAddressEvm, M>;

impl RollupBlueprint<Native> for NightstreamRollup<Native>
where
    NightstreamRollupSpec<Native>: PluggableSpec,
    <NightstreamRollupSpec<Native> as Spec>::Address: FromVmAddress<EthereumAddress>,
{
    type Spec = NightstreamRollupSpec<Native>;
    type Runtime = Runtime<Self::Spec>;
}

impl RollupBlueprint<WitnessGeneration> for NightstreamRollup<WitnessGeneration>
where
    NightstreamRollupSpec<WitnessGeneration>: PluggableSpec,
    <NightstreamRollupSpec<WitnessGeneration> as Spec>::Address: FromVmAddress<EthereumAddress>,
{
    type Spec = NightstreamRollupSpec<WitnessGeneration>;
    type Runtime = Runtime<Self::Spec>;
}

#[async_trait]
impl FullNodeBlueprint<Native> for NightstreamRollup<Native> {
    type DaService = StorableMidnightDaService;

    type StorageManager =
        NativeStorageManager<MidnightDaSpec, <NightstreamRollupSpec<Native> as Spec>::Storage>;
