...
//!
//! # From CLI argument
//! cargo run --release -p sov-rollup-nightstream --bin decrypt-authority-notes -- \
//!   --fvk "0x..." \
//!   --input '[{"cm":"...","ct":[...],"fvk_commitment":"...","mac":"..."}]'
//!
//! # From stdin
//! echo '[{"cm":"...","ct":[...]}]' | cargo run -p sov-rollup-nightstream --bin decrypt-authority-notes -- --fvk "0x..."
//! ```
//!
//! # Input Format
//!
...
fn main() {
    ...
    let input_json = match &args.input {
        ...
        Some(input) if input.starts_with('[') || input.starts_with('{') => input.clone(),
        Some(path) => {
    ...
    if !errors.is_empty() {
        eprintln!("\n{} note(s) failed to decrypt:", errors.len());
        for (i, e) in errors {
            eprintln!("  Note #{}: {}", i + 1, e);
        }
        std::process::exit(1);
...
mod tests {
    ...
    fn test_decrypt_wrong_fvk_fails() {
        ...
        assert!(result.unwrap_err().contains("FVK commitment mismatch"));
    }
    ...
    fn test_fvk_commitment_computation() {
        let fvk_hex = "897efbb9571eb74650851bff7ca467a51e30b1663de8e290de55900d53e00cab";
        let fvk = parse_hash32(fvk_hex).expect("valid FVK hex");
        let expected = "02b38c9c7f69ff932dc1f80b0c02ebe20e83facb3692020ca9f03ac9b78eca10";
        let computed = fvk_commitment(&fvk);
