...
# Start a local oracle that signs attestations. In mock mode, accept all.
...
# Make aux services use the same DA DB + index DB.
...
echo ""
echo "Starting all services (run_controller.sh)..."
echo ""

exec bash "$SCRIPT_DIR/run_controller.sh" \
  "$@"
