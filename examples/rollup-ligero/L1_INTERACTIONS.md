...
This guide explains how the rollup interacts with the Midnight L1 Bridge contract, and how to run that flow locally.
...
The bridge lifecycle (contract deployment, executor service, indexer access, deposit monitoring) is configured through a single `[sequencer.extension.midnight_bridge]` section and works in any operating mode. Comment out the entire section to disable all L1 interactions.
...
If you prefer to manage the contract and executor manually, omit the `midnight_bridge` section entirely and deploy the contract and start the executor outside of the rollup process.
