...
mod demo_generated {
    include!(concat!(env!("REPO_ROOT"), "/examples/rollup-ligero/autogenerated.rs"));
}
...
SKIP_GUEST_BUILD=1 REPO_ROOT="$REPO_ROOT" cargo build --bin transfer-generator 2>&1 | grep -E "Compiling|Finished" || true
...
SKIP_GUEST_BUILD=1 REPO_ROOT="$REPO_ROOT" cargo build --bin withdraw-generator 2>&1 | grep -E "Compiling|Finished" || true
