...
TRANSFER_GEN_PATH="$GENERATOR_DIR/src/transfer_generator.rs"
cat > "$TRANSFER_GEN_PATH" << 'EOFRS'
...
mod demo_generated {
    include!("REPO_ROOT_PLACEHOLDER/examples/rollup-ligero/autogenerated.rs");
}
...
sed -i.bak "s|REPO_ROOT_PLACEHOLDER|$REPO_ROOT|g" "$TRANSFER_GEN_PATH"
rm -f "$TRANSFER_GEN_PATH.bak"
...
path = "src/transfer_generator.rs"
EOF
fi
...
WITHDRAW_GEN_PATH="$GENERATOR_DIR/src/withdraw_generator.rs"
cat > "$WITHDRAW_GEN_PATH" << 'EOFRS'
use anyhow::{Context, Result};
use borsh;
use hex;
use demo_stf::runtime::{Runtime, RuntimeCall};
...
# Replace placeholder with actual repo root path
sed -i.bak "s|REPO_ROOT_PLACEHOLDER|$REPO_ROOT|g" "$WITHDRAW_GEN_PATH"
rm -f "$WITHDRAW_GEN_PATH.bak"
...
path = "src/withdraw_generator.rs"
EOF
fi
