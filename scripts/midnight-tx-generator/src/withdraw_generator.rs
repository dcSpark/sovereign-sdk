...
fn main() -> Result<()> {
    ...
    let (viewer_atts, view_attestations_pub, view_ciphertexts) = if let Some(fvk) = authority_fvk {
        ...
        if n_out == 1 {
            ...
            if fvk_bundle.is_some() {
                println!("POOL_FVK_PK configured: generating viewer attestation for change output (with pool signature)");
            } else {
                println!("AUTHORITY_FVK configured (deprecated): generating viewer attestation for change output");
            }
        ...
        println!("No FVK configured (set POOL_FVK_PK or AUTHORITY_FVK): withdrawal will not include viewer attestation");
        (None, None, None)
    };
    ...
    if let Some(ref bundle) = fvk_bundle {
        if viewer_atts.is_some() {
            ...
            let bl_checks = if withdraw_amount > 0 { 1 } else { 2 };
